 ...
 *
 * Resources: Google, canvas resources, and previous java homework
 */
public class LinkedQueue<T> {
    ...
    public void enqueue(T data) {
        ...
        LinkedNode<T> n = new LinkedNode<>(data);
        tail.setNext(n);
        tail = n;
        size += 1;
    }
    ...
    public T dequeue() {
        if (size == 0) {
            throw new NoSuchElementException("cannot dequeue because structure is already empty");
        }
    ...
    public T peek() {
        if (size == 0) {
            throw new NoSuchElementException("cannot peak at top because structure is already empty");
        }
        return head.getData();
    }
